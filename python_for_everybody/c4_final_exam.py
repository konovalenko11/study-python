...
class WOFHumanPlayer(WOFPlayer):
    ...
    def __init__(self, in_name):
        WOFPlayer.__init__(self, in_name)

    def getMove(self, cat, phrase, gues_phrase):
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def __init__(self, in_name, in_difficulty):
        WOFPlayer.__init__(self, in_name)
    ...
    def getPossibleLetters(self, guessed):
        ...
        for let in LETTERS:
            ...
            if let in VOWELS and self.prizeMoney < VOWEL_COST:
                continue

            if let not in guessed:
                pos_letters.append(let)
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        if len(pos_letters) == 0:
            return 'pass'

        is_good_move = self.smartCoinFlip()

        good_move_let = ''
        for let in self.SORTED_FREQUENCIES:
            if let in pos_letters:
                good_move_let = let
                break

        if is_good_move:
            return good_move_let
        else:
